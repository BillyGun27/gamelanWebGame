...
Scene.Saron.prototype = {
    ...
    create : function(){
        ...
        this.board.scale.setTo(window.devicePixelRatio * ScalePercentWidth , window.devicePixelRatio * ScalePercentHeight);   

      
        sound[0] = game.add.audio('saron'+(0+1) ) ;
        sound[0].addMarker("ting", 1 , 2 );
        this.btnSaron[0] = game.add.button( (this.game.world.centerX/2) , this.game.world.centerY , 'plaq' ,  this.actionOnClick , {pl : 0});
        this.btnSaron[0].anchor.setTo(0.5);  
        this.btnSaron[0].scale.setTo(window.devicePixelRatio * ScalePercentWidth , window.devicePixelRatio * ScalePercentWidth);   
        this.btnSaron[0].input.useHandCursor = false;

        for (var i = 1 ; i < 7 ;i++){
            sound[i] = game.add.audio('saron'+(i+1) ) ;
            sound[i].addMarker("ting", 1 , 2 );
            this.btnSaron[i] = game.add.button( (this.game.world.centerX/2)+this.btnSaron[0].width*i , this.game.world.centerY , 'plaq' ,  this.actionOnClick , {pl : i});
            this.btnSaron[i].anchor.setTo(0.5);  
            this.btnSaron[i].scale.setTo(window.devicePixelRatio * ScalePercentWidth , window.devicePixelRatio * ScalePercentWidth);   
            this.btnSaron[i].input.useHandCursor = false;
        }
